...
/// Mumble's user-editable lexicon, persisted as a plain text file.
///
/// Plain text keeps the format inspectable and easy to edit outside the app. The format is one
/// entry per line:
///
/// ```
/// Mumble
...
/// # off: old phrase -> Preferred Phrase
/// ```
///
...
///
/// The file is watched, so hand edits appear in the UI without a relaunch.
@MainActor
@Observable
final class DictionaryStore {
    ...
    private static let header = """
        # Mumble dictionary
        #
        #   Mumble                    a term — the engine is told this word exists
        #   voice memo -> Voice Memo  a correction — when you hear X, write Y
    ...
    /// Watches for hand edits and refreshes the native view without a relaunch.
    ///
